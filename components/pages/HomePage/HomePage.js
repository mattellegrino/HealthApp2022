...
const getTodaySteps = (today) => {
    fetch(`${global.endpoint}/api/patients/${global.id}/activities/steps?startDate=${today}&endDate=${today}`)
        .then((response) => response.json())
        .then((json) =>{
...
const getTodayHrValue = (today) => {
        fetch(`${global.endpoint}/api/patients/${global.id}/hrs/rest?startDate=${today}&endDate=${today}`)
            .then((response) => response.json())
            .then((json) =>{
                if(json[json.length -1] !== undefined)
   ...
   const getTodaySleepValue= (today) => {
       fetch(`${global.endpoint}/api/patients/${global.id}/sleep/duration?startDate=${today}&endDate=${today}`)
           .then((response) => response.json())
           .then((json) =>{
      ...
      >
        <View style={{ flex: 1, paddingTop: 10 }}>
          <Text>
            ...
            <Text style={s.header(1, "medium")}>{props.username}</Text>
          </Text>
      ...
      >
        <View style={{ flex: 1, flexDirection: "column" }}>
          ...
          <Pressable
            style={{ flex: 3.5 }}
            onPress={() =>
              navigation.navigate("Attività fisica", {
