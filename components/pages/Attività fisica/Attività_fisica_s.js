...
export default function Attività_fisica_s({ route }) {
  ...
  const plus = () => {
    switch (isSelected) {
      case "Giorno": {
        ...
        getSteps(dayafterforapi, dayafterforapi)
          .then((_steps) => {
        ...
        getHrValues(dayafterforapi, dayafterforapi)
          .then((_hrValues) => {
      ...
      case "Mese": {
        ...
        let variablefirstmonthday = new Date(
          monthafter.getFullYear(),
          monthafter.getMonth(),
          1
        );
        setVariableFirstMonthDay(variablefirstmonthday);
        let variablelastmonthday = new Date(
          monthafter.getFullYear(),
          monthafter.getMonth() + 1,
          0
        );
        setVariableLastMonthDay(variablelastmonthday);

        let firstdaymonthafter = new Date(
          monthafter.getFullYear(),
          monthafter.getMonth(),
          1
        );
        let firstdaymonthafterapi = formatDate(firstdaymonthafter);
        let lastdaymonthafter = new Date(
          monthafter.getFullYear(),
          monthafter.getMonth() + 1,
          0
        );
        let lastdaymonthafterapi = formatDate(lastdaymonthafter);
        ...
        let number_days_of_month =
          lastdaymonthafter.getDate() - firstdaymonthafter.getDate();
        fillDates(
          firstdaymonthafterapi,
          lastdaymonthafterapi,
          "steps",
          "month",
  ...
  const minus = () => {
    switch (isSelected) {
      case "Giorno": {
        ...
        getSteps(daybeforeforapi, daybeforeforapi)
          .then((_steps) => {
            setNumStepsDone(_steps[0].steps);
            handleColorNumStepsDone(_steps[0].steps);
          })
        ...
        getHrValues(daybeforeforapi, daybeforeforapi)
          .then((_hrValues) => {
            setHrRest(_hrValues[0].rest);
          })
          .catch((err) => setHrRest(0));
      ...
      case "Mese": {
        ...
        let firstdaymonthbefore = new Date(
          monthbefore.getFullYear(),
          monthbefore.getMonth(),
          1
        );
        let firstdaymonthbeforeapi = formatDate(firstdaymonthbefore);
        let lastdaymonthbefore = new Date(
          monthbefore.getFullYear(),
          monthbefore.getMonth() + 1,
          0
        );
        let lastdaymonthbeforeapi = formatDate(lastdaymonthbefore);

        let number_days_of_month =
          lastdaymonthbefore.getDate() - firstdaymonthbefore.getDate();
        fillDates(
          firstdaymonthbeforeapi,
          lastdaymonthbeforeapi,
          "steps",
          "month",
          number_days_of_month
        );
  ...
  const media = (array) => {
    ...
    if (lunghezza !== 0) return sum / lunghezza;
    else return 0;
  };

  const handleColorNumStepsDone = (value) => {
    if (value < redthreshold) setColorNumStepsDone("red");
    else if (value >= redthreshold && value < orangethreshold) {
      setColorNumStepsDone("orange");
    } else if (value >= orangethreshold && value < yellowthreshold) {
  ...
  const handleColorHrRest = (value) => {
    ...
    else if (value <= hr_redthreshold && value > hr_orangethreshold) {
      setColorHrRest("orange");
    } else if (value <= hr_orangethreshold && value > hr_yellowthreshold) {
  ...
  async function getSteps(startDate, endDate) {
    const response = await fetch(
      `${global.endpoint}/api/patients/${global.id}/activities/steps?startDate=${startDate}&endDate=${endDate}`
    );
  ...
  async function getHrValues(startDate, endDate) {
    const response = await fetch(
      `${global.endpoint}/api/patients/${global.id}/hrs/rest?startDate=${startDate}&endDate=${endDate}`
    );
  ...
  const fillDates = async (
    ...
    if (activity_type === "steps") {
      getSteps(firstweekdayapi, lastweekdayapi)
        .then((_stepsweek) => {
      ...
      getHrValues(firstweekdayapi, lastweekdayapi)
        .then((hrweek) => {
  ...
  useEffect(() => {
    ...
    getHrValues(dayforapi, dayforapi)
      .then((_hrValues) => {
        setHrRest(_hrValues[0].rest), setLineDataDayHr(hr_rest);
  ...
  useEffect(() => {
    switch (isSelected) {
      ...
      case "Mese":
        ...
        let lastmonthdayapi = formatDate(lastmonthday);
        let number_days_of_month =
          lastmonthday.getDate() - firstmonthday.getDate();
        fillDates(
          firstmonthdayapi,
          lastmonthdayapi,
          "steps",
  ...
  return (
    <View style={styles.container_main}>
          ...
          <View style={{ flex: 0.5 }}>
            ...
            >
              ...
              >
                ...
                <View style={styles.details_sleep}>
                  <Text style={s.smalltext("regular")}>A riposo</Text>
                  <View>
                    <View
                      style={{
                        flex: 0,
                    ...
                    >
                      ...
                      <Text style={s.smalltext("regular")}>Bpm</Text>
                    </View>
                  </View>
                  {tipoUtente == "sperimentale" && (
                    <Text style={[s.body("medium"), { margin: "1%" }]}>
            ...
            {isSelected != "Giorno" && (
              <LineChart
                ...
                startFillColor={
                  tipoUtente == "sperimentale" ? color_hr_rest : "grey"
                }
                endFillColor={
                  tipoUtente == "sperimentale" ? color_hr_rest : "grey"
                }
                ...
                showReferenceLine1
                maxValue={150}
                referenceLine1Position={hr_rest}
                referenceLine1Config={{
                  color: "gray",
                  labelText: "Media",
                  dashWidth: 2,
      ...
      <GestureRecognizer
        style={styles.container_swipe_gestures}
        onSwipeLeft={() => plus()}
        onSwipeRight={() => minus()}
      >
