...
export default function Attività_fisica_s({ route }) {
  ...
  async function getSteps(startDate,endDate) {
    const response = await fetch(`${global.endpoint}/api/patients/${global.id}/activities/steps?startDate=${startDate}&endDate=${endDate}`);
  ...
  async function getHrValues(startDate,endDate) {
    const response = await fetch(`${global.endpoint}/api/patients/${global.id}/hrs/rest?startDate=${startDate}&endDate=${endDate}`);
