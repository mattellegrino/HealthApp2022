...
*/
  ...
  getPatientById = () => {
    fetch(`http://${global.matteo}:8080/api/patients/1`)
  ...
  return (
    <View style={{ padding: 20 }}>
      <Text style={{ fontWeight: "700" }}>
        Il server esterno ha risposto con questo contenuto:
      </Text>
      {isLoading ? (
        <ActivityIndicator />
      ) : (
      ...
      <Text style={{ fontWeight: "700" }}>
        Il server interno ha risposto con questo contenuto (JSON FILE):
      </Text>
      {isLoadingPatient ? (
        <ActivityIndicator />
      ) : (
        <Text>{JSON.stringify(patient)}</Text>
      )}
      <Text>
        {" "}
